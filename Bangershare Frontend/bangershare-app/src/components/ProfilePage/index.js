...
import { Grid, Typography, Divider, IconButton, Avatar, Tooltip } from "@material-ui/core";
        ...
        <Grid container direction="row" item xs={12} spacing={3}>
          ...
          <Grid item >
            <div style={{ paddingTop: "35px", paddingLeft: "15px" }}>
              {username && username !== localStorage.getItem("username") && !isFetching
                ?
                <>
                  {(() => {
                    if (sentRequests.find(request => request.receiverUsername === username) !== undefined && sentRequests.length > 0) {
                      return (
                        <Tooltip title="Cancel Request" onClick={() => deleteRequest(username)}>
                          <IconButton color="primary">
                            <Avatar style={{ backgroundColor: "#ff3d00" }}>
                              <PersonAddDisabledIcon style={{ fill: "white" }} />
                            </Avatar>
                          </IconButton>
                        </Tooltip>
                      )
                    }
                    else if (friends.find(friend => friend.username === username) !== undefined) {
                      return (
                        <>
                          ...
                          <Tooltip title="Delete Friend">
                            <IconButton color="primary" onClick={() => deleteRequest(username)}>
                              <Avatar style={{ backgroundColor: "#ff3d00" }}>
                                <PersonAddDisabledIcon style={{ fill: "white" }} />
                              </Avatar>
                            </IconButton>
                    ...
                    } else if (pendingRequests.find(request => request.senderUsername === username) !== undefined && pendingRequests.length > 0) {
                      return (
                        <>
                          <Tooltip title="Accept Request">
                            <IconButton color="primary" onClick={() => acceptRequest(username)}>
                              <Avatar style={{ backgroundColor: "#7d12ff" }}>
                      ...
                      return (
                        <Tooltip title="Add Friend">
                          <IconButton color="primary" onClick={() => addFriend(username)}>
                            <Avatar style={{ backgroundColor: "#7d12ff" }}>
              ...
              {profilePlaylist && profilePlaylist.length > 0 ? (
                <>
                  <Grid item xs={12} style={{ paddingTop: "6vh" }}>
                    <Typography variant="h5" style={{ paddingLeft: "20px" }}>{username}'s Playlists</Typography>
                    {createPlaylist(
                  ...
                  <Grid item xs={12} style={{ paddingTop: "6vh" }}>
                    <Typography variant="h5" style={{ paddingLeft: "20px" }}>Playlists {username} follows</Typography>
                    {createPlaylist(
                      profilePlaylist.filter(
                  ...
                  <Grid item xs={12}>
                    <Typography variant="h4" style={{ paddingLeft: "5vw", paddingTop: "24px" }}>Sorry the user has no playlists</Typography>
                  </Grid>
                )}
            </>
            : <p>loading</p>}
